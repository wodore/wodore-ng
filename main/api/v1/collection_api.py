...
class CollectionsAPI(Resource):
    ...
    def get(self):
        ...
        collections_future = Collection.query() \
            .order(-Collection.created) \
            .fetch_page_async(10, start_cursor=args.cursor)

        total_count_future = Collection.query().count_async(keys_only=True)
        collections, next_cursor, more = collections_future.get_result()
        collections = [u.to_dict(include=Collections.get_public_properties()) for u in collections]
